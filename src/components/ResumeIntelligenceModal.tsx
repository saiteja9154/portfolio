      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          ...
          <motion.div
            ...
            className="relative w-full max-w-4xl bg-[#050816]/95 border border-indigo-500/25 rounded-2xl shadow-[0_0_50px_rgba(0,217,255,0.25)] overflow-hidden max-h-[85vh] flex flex-col z-10 backdrop-blur-xl"
          >
            ...
            {/* Header */}
            <div className="relative flex items-center justify-between p-6 border-b border-indigo-500/10 bg-slate-900/40">
              <div className="flex items-center space-x-3">
                <div className="p-2 bg-indigo-500/10 rounded-lg border border-indigo-500/20 text-[#00D9FF]">
                  <Cpu className="w-5 h-5 animate-pulse" />
                </div>
                <div>
                  <h2 className="text-xl font-bold text-white font-display flex items-center gap-1.5">
                    RESUME INTELLIGENCE REPORT <span className="text-xs py-0.5 px-2 bg-indigo-500/20 text-[#00D9FF] border border-indigo-500/20 rounded-full">REALTIME SYNAPSE AUDIT</span>
                  </h2>
              ...
              <button
                ...
                onClick={onClose}
                className="p-1.5 text-slate-400 hover:text-white bg-white/5 hover:bg-[#00D9FF]/20 border border-white/10 hover:border-[#00D9FF]/40 rounded-lg transition-all cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8 select-none">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="bg-slate-900/60 border border-indigo-500/10 p-4 rounded-xl flex flex-col justify-between">
                  ...
                  <div className="w-full bg-slate-950 h-1.5 rounded-full mt-2 overflow-hidden border border-indigo-500/10">
                    <div className="bg-[#00D9FF] h-full w-[81%] rounded-full" />
                  </div>
                </div>
                <div className="bg-slate-900/60 border border-indigo-500/10 p-4 rounded-xl flex flex-col justify-between">
                ...
                <div className="bg-slate-900/60 border border-indigo-500/10 p-4 rounded-xl flex flex-col justify-between">
                  ...
                  <span className="text-2xl font-bold font-display text-[#00D9FF] mt-1">Excellent</span>
                  <div className="w-full bg-slate-950 h-1.5 rounded-full mt-2 overflow-hidden border border-indigo-500/10">
                    <div className="bg-[#10B981] h-full w-[95%] rounded-full" />
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-slate-900/40 border border-indigo-500/10 p-5 rounded-xl space-y-4">
                  <div className="flex items-center space-x-2 text-[#00D9FF] font-bold border-b border-indigo-500/10 pb-2">
                  ...
                  <div className="space-y-4">
                    {INTERNSHIP_LIST.map((ist, idx) => (
                      <div key={idx} className="border-l-2 border-[#00D9FF]/30 pl-3">
                        <div className="flex justify-between items-start">
                          <h4 className="text-xs font-semibold text-white">{ist.organization}</h4>
                          <span className="text-[9px] px-1 bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 rounded font-mono">{ist.duration}</span>
                        </div>
                        <p className="text-[11px] text-slate-400">{ist.role}</p>
                        <div className="flex flex-wrap gap-1 mt-1.5">
                          {ist.technologies.slice(0, 3).map((tech, tIdx) => (
                            <span key={tIdx} className="text-[8px] bg-slate-950 border border-indigo-500/10 text-slate-300 px-1.5 py-0.5 rounded font-mono">{tech}</span>
                          ))}
                        </div>
                      </div>
                    ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-slate-900/40 border border-indigo-500/10 p-5 rounded-xl space-y-4">
                  <div className="flex items-center space-x-2 text-[#10B981] font-bold border-b border-indigo-500/10 pb-2">
                  ...
                  <div className="space-y-3">
                    {CERTIFICATION_LIST.map((cert, idx) => (
                      <div key={idx} className="flex justify-between items-center bg-slate-950/60 border border-indigo-500/10 p-2 rounded-lg">
                ...
                <div className="bg-slate-900/40 border border-indigo-500/10 p-5 rounded-xl space-y-4">
                  ...
                  <div className="space-y-3">
                    {PROJECT_LIST.map((proj, idx) => (
                      <div key={idx} className="bg-slate-950/60 border border-indigo-500/10 p-2.5 rounded-lg space-y-2">
                        <div className="flex justify-between items-center">
                          <span className="text-xs text-slate-200 font-bold font-display">{proj.name}</span>
                          <span className="text-[8px] font-mono px-1 py-0.5 bg-indigo-950 text-indigo-300 border border-indigo-500/30 rounded uppercase">{proj.category}</span>
                        </div>
                        <p className="text-[10px] text-slate-400 leading-relaxed font-sans">{proj.description}</p>
                        <div className="flex flex-wrap gap-1">
                          {proj.technologies.map((t, tIdx) => (
                            <span key={tIdx} className="text-[8px] font-mono bg-slate-900 border border-indigo-500/10 text-slate-300 px-1 py-0.2 rounded">{t}</span>
                          ))}
                        </div>
                      </div>
                    ))}
            ...
            <div className="p-4 bg-slate-950 border-t border-indigo-500/10 flex items-center justify-between text-[10px] font-mono text-slate-400">
